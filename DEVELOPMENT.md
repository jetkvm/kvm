...
```plaintext
In file included from /workspaces/kvm/internal/hal/native/cgo/ctrl.c:15:
/workspaces/kvm/internal/hal/native/cgo/ui_index.h:4:10: fatal error: ui/ui.h: No such file or directory
 #include "ui/ui.h"
          ^~~~~~~~~
compilation terminated.
```

This means that your system didn't create the directory-link to from _./internal/hal/native/cgo/ui_ to ./internal/hal/native/eez/src/ui when the repository was checked out. You can verify this is the case if _./internal/hal/native/cgo/ui_ appears as a plain text file with only the textual contents:
...
```bash
   ...
   git restore internal/hal/native/cgo/ui
```
...
```bash
git diff --cached --diff-filter=d > ../internal/hal/native/cgo/lvgl-minify.patch && \
git diff --name-only --diff-filter=D --cached > ../internal/hal/native/cgo/lvgl-minify.del
```
