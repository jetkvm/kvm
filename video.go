...
// The first acquirer selects codecType, starts the native video stream and
// pauses the HDMI sleep ticker; subsequent acquirers from different consumers
// are recorded without touching the underlying stream, so codecType is only
// honoured by the consumer that actually starts the encoder. Idempotent per
// consumer key — re-acquiring an already-held key is a no-op.
func acquireVideoStream(consumer string, codecType int) {
	...
	if len(videoConsumers) == 1 {
		// The codec must be selected before the encoder starts, otherwise the
		// first frames come out in the previous consumer's codec.
...
func rpcResumeVideo(s *Session) error {
	acquireVideoStreamForSession(s)
	return nil
}
