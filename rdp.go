...
func initRDPServer() error {
	...
	if config.RDPUseTLS && config.TLSMode != "" && config.TLSMode != "disabled" {
		tlsConfig := &tls.Config{
			GetCertificate: getCertificate,
			MinVersion:     tls.VersionTLS12,
			// CredSSP/NLA requires TLS 1.2 - TLS 1.3 is not compatible with CredSSP
			// because CredSSP relies on TLS session binding which changed in TLS 1.3
