...
UNSIGNED="$OUT/JetKVM-unsigned.apk"
ALIGNED="$OUT/JetKVM-aligned.apk"

case "$MODE" in
  debug)
    SIGNED="$OUT/JetKVM-debug.apk"
    KEYSTORE="${KEYSTORE:-$ROOT/jetkvm-android-debug.keystore}"
    KEY_ALIAS="${KEY_ALIAS:-jetkvm-android}"
    KEY_DNAME="${KEY_DNAME:-CN=JetKVM Android Debug,O=JetKVM}"
    ;;
  release)
    SIGNED="$OUT/JetKVM-release.apk"
    KEYSTORE="${KEYSTORE:-$ROOT/jetkvm-android-release.keystore}"
    KEY_ALIAS="${KEY_ALIAS:-jetkvm-android-release}"
    KEY_DNAME="${KEY_DNAME:-CN=JetKVM Android Release,O=JetKVM}"
    ;;
  *)
