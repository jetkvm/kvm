...
func (u *UsbGadget) Init() error {
	...
	udcs := getUdcs()
	if len(udcs) < 1 {
		logging.LogInfo(u.getLoggingContext(), "no udc found, skipping USB stack init")
		return nil
	}
...
func (u *UsbGadget) UpdateGadgetConfig() error {
	...
	if err != nil {
		return logging.LogError(u.getLoggingContext(), err, "unable to update gadget config")
	}
...
func (u *UsbGadget) configureUsbGadget(resetUsb bool) error {
	return u.WithTransaction(func(tx *UsbGadgetTransaction, u *UsbGadget) error {
		tx.MountConfigFS()
		tx.CreateConfigPath(u.configC1Path)
		tx.WriteGadgetConfig(u)
		if resetUsb {
