...
int main(void)
{
    ...
    assert(automatic.target == 1000 && automatic.maximum == 1500);
    automatic = video_bitrate_for(0, 1920, 1080, 1);
    assert(automatic.target == 200 && automatic.maximum == 200);
    ...
    assert(automatic.maximum == 500 && automatic.target <= automatic.maximum);
    return 0;
}
