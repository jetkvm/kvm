...
# test_unsigned_specific_ota.sh - Run unsigned custom-version OTA E2E test
#
# This wrapper configures environment variables and runs
# z-ota-specific-version-unsigned.spec.ts, which deploys the baseline binary,
# starts a mock server for the target binary, and validates the custom-version
# OTA path.
#
# Usage: ./test_unsigned_specific_ota.sh <device_ip> <baseline_path> <target_path> <target_version>
...
if NODE_NO_WARNINGS=1 npx playwright test z-ota-specific-version-unsigned.spec.ts; then
    echo ""
    echo -e "${GREEN}✓ OTA specific-version unsigned test passed${NC}"
    TEST_RESULT=0
else
    echo ""
    echo -e "${RED}✗ OTA specific-version unsigned test failed${NC}"
    TEST_RESULT=1
fi
