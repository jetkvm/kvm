...
export const Keycap = memo(function Keycap({ transportKey, onPress, isPressed }: KeycapProps) {
  const { x, y, w, h, shape, legends, scancode, dead, homing, decal, color, textColor } = transportKey;
  ...
  return (
    ...
    >
      ...
      {legends.altgr      && <span className="legend altgr"       aria-hidden="true">{legends.altgr}</span>}
      {legends.shiftAltgr && <span className="legend shift-altgr" aria-hidden="true">{legends.shiftAltgr}</span>}
    </div>
  );
});
